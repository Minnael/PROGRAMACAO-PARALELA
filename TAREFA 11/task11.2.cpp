...
const int NX = 1000;     // TAMANHO DO GRID NA DIREÇÃO X
const int NY = 1000;     // TAMANHO DO GRID NA DIREÇÃO Y
const int STEPS = 500;  // NÚMERO DE PASSOS DE TEMPO
...
int main() {
    ...
    for (int passo = 0; passo < STEPS; passo++) { // LOOP SOBRE OS PASSOS DE TEMPO
        ...
        if (passo % 50 == 0)                     // SALVA OS DADOS A CADA 50 PASSOS
            salvar_csv(u, passo);                // ESCREVE O ESTADO ATUAL EM UM ARQUIVO CSV
    }
